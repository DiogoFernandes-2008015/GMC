...
<body>
    ...
    <script>
        ...
        async function buscar() {{
            ...
            try {{
                const res = await fetch(`/api/itens?desc=${{encodeURIComponent(desc)}}&pat=${{encodeURIComponent(pat)}}&local=${{encodeURIComponent(local)}}&conta=${{encodeURIComponent(conta)}}&ficha=${{encodeURIComponent(ficha)}}&limit=500`, {{signal: buscaAbort.signal}});
                const data = await res.json();
