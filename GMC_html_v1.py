...
def processar_relacao_siscofis(texto_bruto: str) -> list[dict]:
    ...
    pattern_linha_nova = re.compile(
        r"^(?P<nome>.*?)\s+(?P<ficha>\d{1,6})\s+(?P<codmat>[A-Z0-9\.-]+)\s+"
...
def sincronizar_pdf_siscofis(itens_pdf: list[dict]) -> dict:
    """Compara o PDF importado com o Banco de Dados e aplica as diferenças por patrimônio."""
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()

    mapa_pdf_pat = {}
    for item in itens_pdf:
        for pat in item["patrimonios"]:
    ...
    pats_pdf = set(mapa_pdf_pat.keys())

    c.execute(
        "SELECT p.nr_patrimonio FROM patrimonios p WHERE p.status_movimentacao = 'Em carga'"
    )
    pats_db = set(row[0] for row in c.fetchall())

    novos = pats_pdf - pats_db
    removidos = pats_db - pats_pdf
    mantidos = pats_pdf & pats_db

    lista_incluidos = []
    lista_removidos = []

    for pat in mantidos:
        pat_info = mapa_pdf_pat[pat]
        pat_val = pat_info["valor"]
        c.execute(
    ...
    for pat in novos:
        pat_info = mapa_pdf_pat[pat]
        item_data = pat_info["item"]
        pat_val = pat_info["valor"]

        c.execute(
            "SELECT id FROM itens WHERE nome_material = ?",
            (item_data["nome_material"],),
        )
        row_item = c.fetchone()
        ...
        if row_item:
            ...
            """,
                (
                    ...
                    item_data["nr_ficha"],
                    item_data["nee_mat"],
                    item_data["conta_contabil"],
                    item_data["valor_unitario"],
                ),
            )
            item_id = c.lastrowid

        c.execute("SELECT id FROM patrimonios WHERE nr_patrimonio = ?", (pat,))
        row_pat = c.fetchone()

        if row_pat:
            c.execute(
                "UPDATE patrimonios SET item_id = ?, status_movimentacao = 'Em carga', valor_individual = ? WHERE id = ?",
                (item_id, pat_val, row_pat[0]),
            )
        else:
            c.execute(
                """
                INSERT INTO patrimonios (item_id, nr_patrimonio, status_conferencia, status_movimentacao, valor_individual)
                VALUES (?, ?, 'Não conferido', 'Em carga', ?)
            """,
                (item_id, pat, pat_val),
            )

        lista_incluidos.append(f"Patrimônio {pat} - {item_data['nome_material']} (R$ {pat_val:.2f})")

    for pat in removidos:
        c.execute(
            "SELECT i.nome_material FROM patrimonios p JOIN itens i ON p.item_id = i.id WHERE p.nr_patrimonio = ?",
            (pat,),
        )
        r_mat = c.fetchone()
        nome_mat = r_mat[0] if r_mat else "Material Desconhecido"

        c.execute(
            "UPDATE patrimonios SET status_movimentacao = 'Fora de carga' WHERE nr_patrimonio = ?",
            (pat,),
        )
        lista_removidos.append(f"Patrimônio {pat} - {nome_mat}")
...
<body>
                ...
                <table id="tabela_dados">
                    <thead>
                        <tr>
                            ...
                            <th>Valor (R$)</th><th>Local</th><th>Últ. Conf.</th><th>Status</th><th>Situação</th><th>Boletim/TEAM</th><th>Fotos</th>
                        </tr>
                    </thead>
                    <tbody></tbody>
                </table>
    ...
    <script>
        ...
        async function buscar() {{
            ...
            const res = await fetch(`/api/itens?desc=${{encodeURIComponent(desc)}}&pat=${{encodeURIComponent(pat)}}&local=${{encodeURIComponent(local)}}&conta=${{encodeURIComponent(conta)}}&ficha=${{encodeURIComponent(ficha)}}`);
            const data = await res.json();
            ...
            data.forEach(item => {{
                ...
                const isIncompleto = (!item.nr_ficha || !item.nee_mat || !item.conta_contabil || !item.local_armazenamento || !item.data_conferencia || !item.foto_objeto || !item.foto_etiqueta);

                let trClass = '';
                if (isForaDeCarga) {{
                    trClass = 'tr-fora-carga';
                }} else if (isIncompleto) {{
                    trClass = 'tr-incompleto';
                }}

                let fotosHtml = '';
                if (item.foto_objeto) fotosHtml += `<a href="${{item.foto_objeto}}" target="_blank">📷 Objeto</a> `;
                else fotosHtml += `<span class="campo-vazio">⚠️ Sem Foto Objeto</span><br>`;

                if (item.foto_etiqueta) fotosHtml += `<a href="${{item.foto_etiqueta}}" target="_blank">🏷️ Etiqueta</a>`;
                else fotosHtml += `<span class="campo-vazio">⚠️ Sem Foto Etiq.</span>`;

                let row = `<tr class="${{trClass}}">
                    <td><b>${{fmtCampo(item.nome_material, 'Descrição')}}</b></td>
                    <td>${{fmtCampo(item.nr_ficha, 'Ficha')}}</td>
                    <td>${{fmtCampo(item.nee_mat, 'NEE')}}</td>
                    <td>${{fmtCampo(item.conta_contabil, 'Conta')}}</td>
                    <td><b>${{fmtCampo(item.nr_patrimonio, 'Patrimônio')}}</b></td>
                    <td><b>${{fmtMoeda(item.valor_individual)}}</b></td>
                    <td>${{fmtCampo(item.local_armazenamento, 'Local')}}</td>
                    <td>${{fmtCampo(item.data_conferencia, 'Data')}}</td>
                    <td><span class="badge ${{badgeClass}}">${{item.status_conferencia || 'Não conferido'}}</span></td>
                    <td><b>${{item.status_movimentacao || 'Em carga'}}</b></td>
        ...
        async function importarPDF(e) {{
            ...
            const etapas = [
                ...
                'Processando itens e patrimônios...',
            ...
            const intervaloEtapas = setInterval(() => {{
                if (etapaAtual < etapas.length - 1) {{
                    etapaAtual++;
                    elStatus.innerText = etapas[etapaAtual];
                }}
            }}, 900);
            ...
            try {{
                ...
                elStatus.innerText = etapas[1];

                const res = await fetch('/api/importar_pdf', {{ 
                    method: 'POST', 
                    body: JSON.stringify({{ pdf_b64: base64 }}) 
                }});
                result = await res.json();
            }} catch (err) {{
                result = {{ success: false, message: 'Erro ao processar o arquivo: ' + err }};
            }} finally {{
            ...
            if(result.success) {{
                ...
                const elemInc = document.getElementById('res_lista_incluidos');
                if (data.incluidos.length > 0) {{
                    elemInc.innerHTML = data.incluidos.map(item => `<div>• ${{item}}</div>`).join('');
                }} else {{
                    elemInc.innerHTML = '<i style="color:#777;">Nenhum novo item incluído.</i>';
                }}

                document.getElementById('res_qtd_removidos').innerText = data.removidos.length;
                const elemRem = document.getElementById('res_lista_removidos');
                if (data.removidos.length > 0) {{
                    elemRem.innerHTML = data.removidos.map(item => `<div>• ${{item}}</div>`).join('');
                }} else {{
                    elemRem.innerHTML = '<i style="color:#777;">Nenhum item removido da carga.</i>';
                }}
...
class RequestHandler(http.server.SimpleHTTPRequestHandler):
            ...
            if parsed_path.path == "/":
                ...
                c.execute("""
                    ...
                    WHERE (i.nr_ficha IS NULL OR i.nr_ficha = '')
                       ...
                       OR (i.foto_objeto IS NULL OR i.foto_objeto = '')
                       OR (i.foto_etiqueta IS NULL OR i.foto_etiqueta = '')
                """)
                inc_row = c.fetchone()
    ...
    def buscar_materiais_para_edicao(self, ficha="", conta=""):
        ...
        query = (
            ...
            " valor_unitario, foto_objeto, foto_etiqueta FROM itens WHERE 1=1"
        )
        params = []
        ...
        try:
            ...
            if self.path == "/api/cadastrar":
                ...
                c.execute(
                    ...
                    WHERE id = ?
                """,
                    (nome, ficha, nee, conta, val_default, item_id),
                )
